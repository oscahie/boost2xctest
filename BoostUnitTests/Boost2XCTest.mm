...
struct xctest_failure_reporter : unit_test_log_formatter {
    ...
    void log_exception_start( std::ostream& os, log_checkpoint_data const& lcd, boost::execution_exception const& ex )
    {
        boost::execution_exception::location const& loc = ex.where();
        if (!loc.m_file_name.empty())
        {
            _sourceFilePath = [[NSString alloc] initWithBytes:loc.m_file_name.begin() length:loc.m_file_name.size() encoding:NSUTF8StringEncoding];
    ...
    void log_entry_start(std::ostream &, log_entry_data const &logEntryData, log_entry_types logEntryType)
    {
        switch (logEntryType)
        {
    ...
    void log_entry_context(std::ostream &, log_level level, const_string context) {};
